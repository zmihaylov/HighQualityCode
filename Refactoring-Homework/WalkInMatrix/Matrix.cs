    ...
    public class Matrix
    {
        ...
        public void Walk()
        {
            ...
            while (true)
            {
                ...
                {
                    bool newPositionFound = this.LookForNewPosition(out position);
                    if (newPositionFound)
                    {
                        counter++;
                        this.matrix[position.Row, position.Col] = counter;
                ...
                while (!this.IsValidPosition(position.Row + delta.Row, position.Col + delta.Col))
                {
        ...
        private bool CanContinue(Position position)
        {
            ...
            {
                if (this.IsValidPosition(position.Row + deltas[i].Row, position.Col + deltas[i].Col))
                {
                    return true;
                }
            }
        ...
        private bool LookForNewPosition(out Position newPosition)
        {
